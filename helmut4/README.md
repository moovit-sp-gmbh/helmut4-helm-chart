...
| rabbitmq.auth.password | string | `"bitte"` | RabbitMQ password. Replace before exposing the service. |
| rabbitmq.auth.username | string | `"admin"` | RabbitMQ user. The Helmut4 frontend ships with `admin` baked in for the STOMP-over-WebSocket connection (browser → `/ws`), so this must stay `admin` unless you can also rebuild the frontend bundle. |
